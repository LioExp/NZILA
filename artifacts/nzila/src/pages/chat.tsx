...
export default function Chat() {
  ...
  const handleSend = (text: string) => {
    ...
    addMessage({ id: uuidv4(), role: "user", content: text, createdAt: new Date() });
    setInput("");
    chatMutation.mutate({
      data: {
        message: text,
      ...
      <header className="px-4 py-3 border-b border-border/40 bg-background/90 backdrop-blur-md z-10 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-foreground">Nzila Chat</span>
          </div>
        </div>
                    ...
                    {msg.role === "assistant" && msg.source && (
                      <div className="flex items-center gap-2 px-1">
                        <span className={`text-[10px] font-semibold uppercase tracking-wider px-2 py-0.5 rounded border ${
                          ...
                          {msg.source.includes("dataset") ? "Dataset" : "IA"}
                        </span>
                        {msg.matchedGiria && (
